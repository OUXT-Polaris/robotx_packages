...
{
  roi_array_pub_ = nh_.advertise<robotx_msgs::RegionOfInterest2DArray>(
      ros::this_node::getName() + "/result", 10);
  XmlRpc::XmlRpcValue parameters;
  nh_.getParam(ros::this_node::getName(), parameters);
  for (auto threshold_param_itr = parameters.begin();
       threshold_param_itr != parameters.end(); ++threshold_param_itr) {
    hsv_threshold threashold_param(threshold_param_itr->first,
                                   threshold_param_itr->second["min_h"],
                                   threshold_param_itr->second["max_h"],
                                   threshold_param_itr->second["min_s"],
                                   threshold_param_itr->second["max_s"],
                                   threshold_param_itr->second["min_v"],
  ...
  image_sub_ = it_.subscribe(ros::this_node::getName() + "/image_raw", 1,
                             &hsv_buoy_detector::image_callback, this);
}
...
void hsv_buoy_detector::image_callback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  for (int i = 0; i < masked_images_.size(); i++) {
    ...
    threashold_params[i].get_threshold(min_h, max_h, min_s, max_s, min_v,
                                       max_v);
    // cv::inRange(hsv_image, cv::Scalar(min_h, min_s, min_v, 0) ,
    // cv::Scalar(max_h, max_s, max_v, 0),
    // masked_images_[i]);
    if (min_h < max_h) {
      cv::inRange(hsv_image, cv::Scalar(min_h, min_s, min_v, 0),
                  cv::Scalar(max_h, max_s, max_v, 0), masked_images_[i]);
    }
    else {
      cv::Mat lower_image;
      cv::inRange(hsv_image, cv::Scalar(min_h, min_s, min_v, 0),
                  cv::Scalar(255, max_s, max_v, 0), lower_image);
      cv::Mat upper_image;
      cv::inRange(hsv_image, cv::Scalar(0, min_s, min_v, 0),
                  cv::Scalar(max_h, max_s, max_v, 0), upper_image);
      cv::bitwise_or(lower_image, upper_image, masked_images_[i]);
    }
    std::vector<std::vector<cv::Point> > contours;
    cv::findContours(masked_images_[i], contours, CV_RETR_EXTERNAL,
                     CV_CHAIN_APPROX_NONE);
    double min_area, max_area;
    threashold_params[i].get_area_threshold(min_area, max_area);
    // check contour size for ALL contours
    for (std::vector<std::vector<cv::Point> >::const_iterator itr =
             contours.begin();
      ...
      if (max_area >= area && area >= min_area) {
        ...
        // object2d_msg.boundingbox.corner_point_1[0] = bbox_rect.x +
        // bbox_rect.width;
        // object2d_msg.boundingbox.corner_point_1[1] = bbox_rect.y +
        // bbox_rect.height;
