...
void passthough_filter::pointcloud_callback_(const sensor_msgs::PointCloud2ConstPtr& msg)
{
    ...
    if(param_.mode == remove)
    {
        for(pcl::PointCloud<pcl::PointXYZI>::iterator i = cloud->begin(); i!= cloud->end(); i++)
        {
            ...
            bool valid_y;
            bool valid_z;
            if(param_.min_x == 0 && param_.max_x == 0){valid_x = true;}
            else
            {
                if(param_.min_x > (*i).x || param_.max_x < (*i).x){valid_x = true;}
                else{valid_x = false;}
            }
            if(param_.min_y == 0 && param_.max_y == 0){valid_y = true;}
            else
            {
                if(param_.min_y > (*i).y || param_.max_y < (*i).y){valid_y = true;}
                else{valid_y = false;}
            }
            if(param_.min_z == 0 && param_.max_z == 0){valid_z = true;}
            else
            {
                if(param_.min_z > (*i).z || param_.max_z < (*i).z){valid_z = true;}
                else{valid_z = false;}
            }
