...
object_bbox_extractor::object_bbox_extractor()
    : it_(nh_), params_(), tf_listener_(tf_buffer_)
{
  roi_array_pub_ = nh_.advertise<robotx_msgs::ObjectRegionOfInterestArray>(
      ros::this_node::getName() + "/object_roi", 1);
  ...
  image_sub_ = it_.subscribe(params_.image_topic, 1,
                             &object_bbox_extractor::image_callback_, this);
  euclidean_cluster_sub_ =
      nh_.subscribe(params_.euclidean_cluster_topic, 1,
                    &object_bbox_extractor::euclidean_cluster_callback_, this);
}

object_bbox_extractor::~object_bbox_extractor() {}
void object_bbox_extractor::image_callback_(
    const sensor_msgs::ImageConstPtr& msg)
{
  ...
  try {
    transform_stamped = tf_buffer_.lookupTransform(
        params_.camera_link, msg->header.frame_id, ros::Time(0));
  }
  ...
  for (int i = 0; i < last_bbox_msg_.boxes.size(); i++) {
    cv::Rect rect;
    if (raycast_to_image(image, last_bbox_msg_.boxes[i], transform_stamped,
                         last_bbox_msg_.header, rect)) {
      ...
      if (params_.enable_roi_image_publisher) {
        ...
        sensor_msgs::ImagePtr roi_image_msg =
            cv_bridge::CvImage(std_msgs::Header(), "bgr8", roi_image)
                .toImageMsg();
  ...
  if (params_.publish_rect_image) {
    sensor_msgs::ImagePtr msg =
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", rect_image).toImageMsg();
    image_pub_.publish(msg);
  }
  roi_array_pub_.publish(roi_array_msg);
}

void object_bbox_extractor::euclidean_cluster_callback_(
    jsk_recognition_msgs::BoundingBoxArray msg)
{
  last_bbox_msg_ = msg;
}

bool object_bbox_extractor::raycast_to_image(
    cv::Mat image,
    ...
    cv::Rect& image_bbox)
{
  ...
  for (int i = 0; i < 4; i++) {
    if (i == 0) {
      ...
      bbox_points[i].point.x =
          object_bbox.pose.position.x + object_bbox.dimensions.x / 2;
      bbox_points[i].point.y =
          object_bbox.pose.position.y + object_bbox.dimensions.y;
      bbox_points[i].point.z =
          object_bbox.pose.position.z + object_bbox.dimensions.z;
    }
    if (i == 1) {
      bbox_points[i].header = header;
      bbox_points[i].point.x =
  ...
  if (image_rect_points[0].y > (image.rows - 1) &&
      image_rect_points[1].y > (image.rows - 1))
    if (image_rect_points[1].y > (image.rows - 1) &&
        image_rect_points[2].y > (image.rows - 1))
      return false;
  if (image_rect_points[0].x < 0 && image_rect_points[1].x < 0)
    if (image_rect_points[1].x < 0 && image_rect_points[2].x < 0) return false;
  if (image_rect_points[0].x > (image.cols - 1) &&
      image_rect_points[1].x > (image.cols - 1))
    if (image_rect_points[1].x > (image.cols - 1) &&
        image_rect_points[2].x > (image.cols - 1))
      return false;
  for (int i = 0; i < 4; i++) {
    if (image_rect_points[i].x < 0) image_rect_points[i].x = 0;
    if (image_rect_points[i].x > (image.cols - 1))
      image_rect_points[i].x = image.cols - 1;
    if (image_rect_points[i].y < 0) image_rect_points[i].y = 0;
    if (image_rect_points[i].y > (image.rows - 1))
      image_rect_points[i].y = image.rows - 1;
  }
  image_bbox.x =
      std::min(std::min(image_rect_points[0].x, image_rect_points[1].x),
               std::min(image_rect_points[2].x, image_rect_points[3].x));
  image_bbox.y =
      std::min(std::min(image_rect_points[0].y, image_rect_points[1].y),
               std::min(image_rect_points[2].y, image_rect_points[3].y));
  image_bbox.width =
      std::max(std::max(image_rect_points[0].x, image_rect_points[1].x),
               std::max(image_rect_points[2].x, image_rect_points[3].x)) -
      image_bbox.x;
  image_bbox.height =
      std::max(std::max(image_rect_points[0].y, image_rect_points[1].y),
