...
{
    roi_array_pub_ = nh_.advertise<robotx_msgs::ObjectRegionOfInterestArray>(ros::this_node::getName()+"/object_roi", 1);
    ...
    geometry_msgs::TransformStamped transform_stamped, std_msgs::Header header, cv::Rect& image_bbox)
{
    ...
    if(image_rect_points[0].y < 0 && image_rect_points[1].y < 0)
        if(image_rect_points[1].y < 0 && image_rect_points[2].y < 0)
            return false;
    if(image_rect_points[0].y > (image.rows-1) && image_rect_points[1].y > (image.rows-1))
        if(image_rect_points[1].y > (image.rows-1) && image_rect_points[2].y > (image.rows-1))
            return false;
    if(image_rect_points[0].x < 0 && image_rect_points[1].x < 0)
        if(image_rect_points[1].x < 0 && image_rect_points[2].x < 0)
            return false;
    if(image_rect_points[0].x > (image.cols-1) && image_rect_points[1].x > (image.cols-1))
        if(image_rect_points[1].x > (image.cols-1) && image_rect_points[2].x > (image.cols-1))
            return false;
    for(int i=0; i<4; i++)
    {
        if(image_rect_points[i].x < 0)
            image_rect_points[i].x = 0;
        if(image_rect_points[i].x > (image.cols-1))
            image_rect_points[i].x = image.cols-1;
        if(image_rect_points[i].y < 0)
            image_rect_points[i].y = 0;
        if(image_rect_points[i].y > (image.rows-1))
            image_rect_points[i].y = image.rows-1;
    }
    image_bbox.x = std::min(std::min(image_rect_points[0].x,image_rect_points[1].x),std::min(image_rect_points[2].x,image_rect_points[3].x));
    image_bbox.y = std::min(std::min(image_rect_points[0].y,image_rect_points[1].y),std::min(image_rect_points[2].y,image_rect_points[3].y));
    image_bbox.width = std::max(std::max(image_rect_points[0].x,image_rect_points[1].x),std::max(image_rect_points[2].x,image_rect_points[3].x)) - image_bbox.x;
    image_bbox.height = std::max(std::max(image_rect_points[0].y,image_rect_points[1].y),std::max(image_rect_points[2].y,image_rect_points[3].y)) - image_bbox.y;
    return true;
}