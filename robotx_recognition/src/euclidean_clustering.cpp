...
euclidean_clustering::euclidean_clustering()
    : conditional_euclidian_clustering_params_(),
      ...
      bbox_params_()
{
  marker_pub_ = nh_.advertise<jsk_recognition_msgs::BoundingBoxArray>(
      ros::this_node::getName() + "/bbox", 1);
  ros::param::param<int>(ros::this_node::getName() + "/min_cluster_size",
                         min_cluster_size_, 10);
  ros::param::param<int>(ros::this_node::getName() + "/max_cluster_size",
                         max_cluster_size_, 1000);
  ros::param::param<bool>(ros::this_node::getName() + "/donwsample/enable",
                          donwsample_, true);
  ros::param::param<double>(
      ros::this_node::getName() + "/donwsample/voxel_grid/leaf_size/x",
      leaf_size_x, 0.01);
  ros::param::param<double>(
      ros::this_node::getName() + "/donwsample/voxel_grid/leaf_size/y",
      leaf_size_y, 0.01);
  ros::param::param<double>(
      ros::this_node::getName() + "/donwsample/voxel_grid/leaf_size/z",
      leaf_size_z, 0.01);
  ros::param::param<std::string>(ros::this_node::getName() + "/input_cloud",
                                 input_cloud_,
                                 ros::this_node::getName() + "/input_cloud");
  ros::param::param<int>(ros::this_node::getName() + "/clustering_method",
                         clustering_method_, CONDITIONAL_EUCLIDIAN_CLUSTERING);
  pointcloud_sub_ = nh_.subscribe(
      input_cloud_, 5, &euclidean_clustering::poincloud_callback, this);
}
...
bool euclidean_clustering::custom_region_growing_function(
...
bool euclidean_clustering::enforce_intensity_similarity(
    const pcl::PointXYZINormal& point_a,
    const pcl::PointXYZINormal& point_b,
    float squared_distance)
{
...
void euclidean_clustering::make_cluster(sensor_msgs::PointCloud2 msg)
{
  if (clustering_method_ == CONDITIONAL_EUCLIDIAN_CLUSTERING) {
    ...
    pcl::NormalEstimation<pcl::PointXYZI, pcl::PointXYZINormal>
        normal_estimation;
    normal_estimation.setInputCloud(pcl_pointcloud);
    pcl::search::KdTree<pcl::PointXYZI>::Ptr search_tree(
        new pcl::search::KdTree<pcl::PointXYZI>);
    normal_estimation.setSearchMethod(search_tree);
    normal_estimation.setRadiusSearch(
        conditional_euclidian_clustering_params_.radius_search);
    normal_estimation.compute(*pcl_cloud_with_normals);

    // Set up a Conditional Euclidean Clustering class
    pcl::ConditionalEuclideanClustering<pcl::PointXYZINormal>
        conditional_euclidean_clustering(true);
    ...
    conditional_euclidean_clustering.getRemovedClusters(small_clusters,
                                                        large_clusters);
    // Generate marker message
    jsk_recognition_msgs::BoundingBoxArray markers;
    markers.header = msg.header;
    for (int i = 0; i < clusters->size(); ++i) {
      pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_pointcloud_clusterd(
          new pcl::PointCloud<pcl::PointXYZI>);
      pcl_pointcloud_clusterd->resize((*clusters)[i].indices.size());
      for (int j = 0; j < (*clusters)[i].indices.size(); ++j) {
        pcl_pointcloud_clusterd->points[j] =
            pcl_pointcloud->points[(*clusters)[i].indices[j]];
      }
      ...
      marker.dimensions.x =
          std::fabs(max_point.x - min_point.x) + bbox_params_.inflation_size;
      marker.dimensions.y =
          std::fabs(max_point.y - min_point.y) + bbox_params_.inflation_size;
