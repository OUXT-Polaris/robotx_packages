...
robotx_msgs::GeographicMap geographic_map_server::generate_geographic_map_()
{
  ...
  try {
    ...
    for (auto node_data_itr = node_data.begin();
         node_data_itr != node_data.end(); ++node_data_itr) {
      robotx_msgs::GeographicPoint geopoint;
      geopoint.node_id =
          (uint64_t)std::stoull(node_data_itr->first.as<std::string>());
      YAML::Node geopos_data =
          node_data[node_data_itr->first.as<std::string>()];
    ...
    for (auto way_data_itr = way_data.begin(); way_data_itr != way_data.end();
         ++way_data_itr) {
      ...
      geoline.start_node_id = (uint64_t)std::stoull(
          geoline_data["start_node_id"].as<std::string>());
      geoline.end_node_id =
          (uint64_t)std::stoull(geoline_data["end_node_id"].as<std::string>());
