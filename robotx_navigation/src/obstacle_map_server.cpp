...
obstacle_map_server::obstacle_map_server() : params_(), tf_listener_(tf_buffer_)
{
  ...
  measurements_ =
      boost::circular_buffer<jsk_recognition_msgs::BoundingBoxArray>(
          params_.buffer_length);
  objects_bbox_sub_ =
      nh_.subscribe(params_.object_bbox_topic, 1,
                    &obstacle_map_server::objects_bbox_callback_, this);
}

obstacle_map_server::~obstacle_map_server() {}
void obstacle_map_server::objects_bbox_callback_(
...
nav_msgs::OccupancyGrid obstacle_map_server::generate_occupancy_grid_map_()
{
  ...
  for (int i = 0; i < measurements_.size(); i++) {
    if (i == measurements_.size() - 1) {
      try {
        ...
        transform_stemped =
            tf_buffer_.lookupTransform(measurements_[i].header.frame_id,
                                       params_.world_frame, ros::Time(0));
        ...
        for (int m = 0; m < measurements_[i].boxes.size(); m++) {
          ...
          tf2::doTransform(pose_stamped_in, pose_stamped_out,
                           transform_stemped);
      ...
      try {
        geometry_msgs::TransformStamped transform_stemped;
        transform_stemped = tf_buffer_.lookupTransform(
            measurements_[i].header.frame_id, params_.world_frame,
  ...
  try {
    transform_stemped = tf_buffer_.lookupTransform(
        params_.world_frame, params_.robot_frame, ros::Time(0));
  }
  catch (tf2::TransformException &ex) {
    ROS_WARN("Could NOT transform  %s", ex.what());
  }
  ...
  for (int i = 0; i < transformed_measurements.size(); i++) {
    for (int m = 0; m < transformed_measurements[i].boxes.size(); m++) {
      ...
      double radius =
          std::max(transformed_measurements[i].boxes[m].dimensions.x,
  ...
  for (int i = 0; i < params_.map_height; i++) {
    for (int m = 0; m < params_.map_width; m++) {
      double x = (double)m * params_.resolution -
                 0.5 * params_.resolution * params_.map_width;
      double y = (double)i * params_.resolution -
                 0.5 * params_.resolution * params_.map_height;
      for (int n = 0; n < objects_data.size(); n++) {
        double distance = std::sqrt(std::pow(objects_data[n][1] - x, 2) +
                                    std::pow(objects_data[n][2] - y, 2));
