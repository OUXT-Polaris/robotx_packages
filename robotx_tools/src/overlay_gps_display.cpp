...
namespace ros_ship_visualization {
  OverlayGpsDisplay::OverlayGpsDisplay() {
    ...
    zoom_property_ =
        new rviz::IntProperty("Zoom", 19, "zoom of map", this, SLOT(updateGooleMapAPIProperty()));
    zoom_property_->setMax(22);
    zoom_property_->setMin(0);
    width_property_ = new rviz::IntProperty("Width", 320, "request map image width", this,
                                            SLOT(updateGooleMapAPIProperty()));
    width_property_->setMax(640);
    width_property_->setMin(0);
    height_property_ = new rviz::IntProperty("Height", 320, "request map image height", this,
                                             SLOT(updateGooleMapAPIProperty()));
    height_property_->setMax(640);
    height_property_->setMin(0);
    scale_property_ =
        new rviz::IntProperty("Scale", 1, "request map image scale", this, SLOT(updateGooleMapAPIProperty()));
    scale_property_->setMax(2);
    scale_property_->setMin(1);
    history_length_property_ =
        new rviz::IntProperty("History Length", 15, "history length", this, SLOT(updateHistoryLength()));
    history_length_property_->setMin(1);
    fix_buffer_ = boost::circular_buffer<sensor_msgs::NavSatFix>(history_length_property_->getInt());
    // api_key_property_ = new rviz::StringProperty("API Key", "", "Google Static Map API Key", this,
    // SLOT(updateGooleMapAPIProperty()));
    maptype_property_ =
        new rviz::EnumProperty("Map Type", "roadmap", "map type", this, SLOT(updateGooleMapAPIProperty()));
    ...
    alpha_property_ =
        new rviz::FloatProperty("Alpha", 0.8, "image alpha", this, SLOT(updateDisplayProperty()));
    alpha_property_->setMax(1);
    alpha_property_->setMin(0);
    position_x_property_ =
        new rviz::IntProperty("Position X", 0, "map image position x", this, SLOT(updateDisplayProperty()));
    position_y_property_ =
        new rviz::IntProperty("Position Y", 0, "map image position y", this, SLOT(updateDisplayProperty()));
    messages_per_plot_property_ =
        new rviz::IntProperty("Message per plot", 5, "message per plot", this, SLOT(updateDisplayProperty()));
  ...
  void OverlayGpsDisplay::processMessage(const sensor_msgs::NavSatFix::ConstPtr& msg) {
    if (msg->header.seq % messages_per_plot_property_->getInt() == 0) {
      ...
      if (check_map_image_file() == false) {
        this->setStatus(rviz::StatusProperty::Level::Error, "MapImageFileNotExist",
      ...
      if (map_image.cols <= 0 || map_image.rows <= 0) {
        this->setStatus(rviz::StatusProperty::Level::Error, "MapImageFileIsInvalidSize",
                        "map image file is invalid size. Check API Key.");
        return;
      }
      this->setStatus(rviz::StatusProperty::Level::Ok, "MapImageFileIsInvalidSize",
                      "map image file is valid size");
      if (!overlay_) {
        static int count = 0;
  ...
  void OverlayGpsDisplay::load_map_downloader_script() {
    std::string map_downloader_path =
        ros::package::getPath("ros_ship_visualization") + "/script/map_downloader.py";
  ...
  bool OverlayGpsDisplay::build_request_url(const sensor_msgs::NavSatFix::ConstPtr& msg,
                                            std::string& request_url) {
    ...
    */
    ...
    std::string center_request =
        "center=" + std::to_string(msg->longitude) + "," + std::to_string(msg->latitude);
    request_url = request_url + center_request;
    std::string markers_request =
        "&markers=color:red%7C" + std::to_string(msg->longitude) + "," + std::to_string(msg->latitude);
    ...
    std::string size_request = "&size=" + std::to_string(width_property_->getInt()) + "x" +
    ...
    for (auto fix_data = fix_buffer_.rbegin(); fix_data != fix_buffer_.rend(); ++fix_data) {
      path_url =
          path_url + "|" + std::to_string(fix_data->longitude) + "," + std::to_string(fix_data->latitude);
    }
