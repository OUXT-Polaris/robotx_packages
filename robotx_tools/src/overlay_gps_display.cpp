...
namespace ros_ship_visualization {
  OverlayGpsDisplay::OverlayGpsDisplay()
  {
    ...
    map_image_path_ =
        ros::package::getPath("ros_ship_visualization") + "/data/map.png";
    ...
    // api_key_property_ = new rviz::StringProperty("API Key", "", "Google
    // Static Map API Key", this,
    // SLOT(updateGooleMapAPIProperty()));
    maptype_property_ =
  ...
  bool OverlayGpsDisplay::download_map(std::string request_url)
  {
    ...
    try {
      PyObject* responce =
          PyObject_Call(map_downloader_function_, args, kw_args);
    }
    catch (...) {
      Py_DECREF(args);
      Py_DECREF(kw_args);
      ...
      const sensor_msgs::NavSatFix::ConstPtr& msg)
  {
    if (msg->header.seq % messages_per_plot_property_->getInt() == 0) {
      ...
      if (map_image.cols <= 0 || map_image.rows <= 0) {
        this->setStatus(rviz::StatusProperty::Level::Error,
                        "MapImageFileIsInvalidSize",
                        "map image file is invalid size. Check API Key.");
        return;
      }
      this->setStatus(rviz::StatusProperty::Level::Ok,
                      "MapImageFileIsInvalidSize",
  ...
  void OverlayGpsDisplay::load_map_downloader_script()
  {
    std::string map_downloader_path =
        ros::package::getPath("ros_ship_visualization") +
        "/script/map_downloader.py";
      ...
      const sensor_msgs::NavSatFix::ConstPtr& msg, std::string& request_url)
  {
    ...
    */
    ...
    // exist");
    std::string center_request = "center=" + std::to_string(msg->longitude) +
    ...
    for (auto fix_data = fix_buffer_.rbegin(); fix_data != fix_buffer_.rend();
         ++fix_data) {
      path_url = path_url + "|" + std::to_string(fix_data->longitude) + "," +
                 std::to_string(fix_data->latitude);
    }
    ...
    if (request_url.size() > MAX_REQUEST_URL_LENGTH) {
      QString message = QString("%1%2")
                            .arg(request_url.size())
                            .arg(" request url is too long");
      ...
      QString message =
          QString("%1%2").arg(request_url.size()).arg("characters");
