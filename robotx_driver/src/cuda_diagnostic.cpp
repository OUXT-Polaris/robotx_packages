...
cuda_diagnostic::cuda_diagnostic() : params_() {
    ...
    updater_.add("gpu-memory-usage", this,
                 &cuda_diagnostic::update_memory_usage_);
    updater_.add("gpu-temperature", this,
                 &cuda_diagnostic::update_temperature_);
}

cuda_diagnostic::~cuda_diagnostic() {}

bool cuda_diagnostic::exec_shell_cmd(const char *cmd,
                                     std::string &stdOut,
                                     int &exitCode) {
...
void cuda_diagnostic::update_temperature_(
    diagnostic_updater::DiagnosticStatusWrapper &stat) {
    std::string device_id_query_cmd =
        "cat /sys/devices/virtual/thermal/thermal_zone*/type";
    std::string device_id_query_result;
    int device_id_query_return_code;
    if (exec_shell_cmd(device_id_query_cmd.c_str(), device_id_query_result,
                       device_id_query_return_code)) {
        std::istringstream stream(device_id_query_result);
        std::string field;
        int target_index = -1;
        int i = 0;
        while (std::getline(stream, field)) {
            if (std::equal(field.begin(), field.end(),
                           params_.gpu_device_name.c_str())) {
                target_index = i;
            }
            i++;
        }
        if (target_index == -1) {
            ...
            std::string get_temperature_cmd =
                "cat /sys/devices/virtual/thermal/thermal_zone*/temp";
            std::string get_temperature_result;
            int get_temperature_return_code;
            if (exec_shell_cmd(get_temperature_cmd.c_str(),
                               get_temperature_result,
                               get_temperature_return_code)) {
                int m = 0;
                ...
                while (std::getline(temperature_stream, temperature_field)) {
                    if (m == target_index) {
                        double temperature =
                            std::stod(temperature_field) / (double)1000;
                        stat.summary(diagnostic_msgs::DiagnosticStatus::OK,
                                     "Get GPU temperature succeed");
                        stat.add("gpu-temperature",
...
void cuda_diagnostic::update_memory_usage_(
    diagnostic_updater::DiagnosticStatusWrapper &stat) {
    size_t free;
    size_t total;
    ...
    if (result != cudaSuccess) {
        ...
        stat.add("gpu-memory-usage",
                 std::to_string((double)free / (double)total * 100) + "%");
    }
}