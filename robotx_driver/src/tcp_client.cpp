...
tcp_client::tcp_client(boost::asio::io_service &io_service,
                       std::string ip_address, int port, int timeout)
    : io_service_(io_service), socket_(io_service), timer_(io_service),
      is_canceled_(false) {
  connection_status_ = false;
  ip_address_ = ip_address;
...
void tcp_client::connect() {
  socket_.async_connect(
      boost::asio::ip::tcp::endpoint(
          boost::asio::ip::address::from_string(ip_address_), port_),
      boost::bind(&tcp_client::on_connect, this,
...
void tcp_client::on_receive(const boost::system::error_code &error,
...
void tcp_client::send(double data) {
  ...
  boost::asio::async_write(
      socket_, boost::asio::buffer(data_arr),
...
void tcp_client::on_send(const boost::system::error_code &error,
                         size_t bytes_transferred) {
  if (error) {
...
void tcp_client::start_receive() {
  boost::asio::async_read(
      socket_, receive_buff_, boost::asio::transfer_all(),
