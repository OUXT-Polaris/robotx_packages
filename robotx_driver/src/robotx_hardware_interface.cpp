...
robotx_hardware_interface::robotx_hardware_interface()
    ...
    if (params_.target == params_.ALL || params_.target == params_.SIMULATION) {
        usv_drive_cmd_pub_ =
            nh_.advertise<robotx_msgs::UsvDrive>("/cmd_drive", 1);
    ...
    fix_sub_ = nh_.subscribe("/fix", 1,
                             &robotx_hardware_interface::fix_callback_, this);
    motor_command_sub_ = nh_.subscribe(
        "/wam_v/motor_command", 1,
...
void robotx_hardware_interface::send_command_() {
    ...
    while (ros::ok()) {
        ...
        mtx_.lock();
        if (params_.target == params_.ALL ||
            params_.target == params_.SIMULATION) {
            if (driving_mode_ == params_.REMOTE_OPERATED) {
                ...
                left_thrust_joint_cmd_.data =
                    last_manual_motor_cmd_msg_.data[1];
                std_msgs::Float64 right_thrust_joint_cmd_;
                right_thrust_joint_cmd_.data =
                    last_manual_motor_cmd_msg_.data[3];
        ...
        if (params_.target == params_.ALL ||
            params_.target == params_.HARDWARE) {
