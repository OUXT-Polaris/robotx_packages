...
heartbeat_publisher::heartbeat_publisher() {
    ...
    nh_.param<std::string>(ros::this_node::getName() + "/ip_address",
                           ip_address_, "127.0.0.1");
    nh_.param<int>(ros::this_node::getName() + "/port", port_, 31400);
    nh_.param<double>(ros::this_node::getName() + "/publish_rate",
                      publish_rate_, 1);
    client_ = new tcp_client(io_service_, ip_address_, port_);
    io_service_.run();
    connection_status_pub_ =
        nh_.advertise<robotx_msgs::TechnicalDirectorNetworkStatus>(
            ros::this_node::getName() + "/connection_status", 1);
    tcp_thread =
        boost::thread(&heartbeat_publisher::publish_heartbeat_message, this);
    heartbeat_sub_ =
        nh_.subscribe(ros::this_node::getName() + "/heartbeat", 1,
...
void heartbeat_publisher::heartbeat_callback(
    const robotx_msgs::Heartbeat::ConstPtr &msg) {
...
void heartbeat_publisher::update_heartbeat_message() {
    tcp_send_msg_ = "RXHRT,";
    tcp_send_msg_ = tcp_send_msg_ + heartbeat_msg_.utc_time_hh +
                    heartbeat_msg_.utc_time_mm + heartbeat_msg_.utc_time_ss +
                    ",";
    tcp_send_msg_ =
        tcp_send_msg_ + std::to_string(heartbeat_msg_.latitude) + ",";
    ...
    tcp_send_msg_ =
        tcp_send_msg_ + std::to_string(heartbeat_msg_.longitude) + ",";
    ...
    tcp_send_msg_ =
        tcp_send_msg_ + std::to_string(heartbeat_msg_.vehicle_mode) + ",";
    tcp_send_msg_ =
        tcp_send_msg_ + std::to_string(heartbeat_msg_.current_task_number);
    tcp_send_msg_ =
        "$" + tcp_send_msg_ + "*" + generate_checksum(tcp_send_msg_.c_str());
}
