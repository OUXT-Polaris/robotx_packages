...
#include <robotx_controller.h>

namespace robotx_controller
{
  RobotXController::RobotXController():command_struct()
  {

  }

  bool RobotXController::init(hardware_interface::VelocityJointInterface* hw,ros::NodeHandle& controller_nh)
  {
    ...
    */
    ...
    sub_twist = controller_nh.subscribe(twist_topic, 1, &RobotXController::twistCallback, this);
    sub_command = controller_nh.subscribe("cmd_vel", 1, &RobotXController::cmdVelCallback, this);
  }

  RobotXController::~RobotXController()
  {
    sub_command.shutdown();
  }

  void RobotXController::starting(const ros::Time& time)
  {
  ...
  void RobotXController::update(const ros::Time& time, const ros::Duration& period)
  {
  ...
  void RobotXController::stopping(const ros::Time& time)
  {

  }

  void RobotXController::setGains(double p, double i, double d, double i_max, double i_min,control_toolbox::Pid& target)
  {
    target.setGains(p,i,d,i_max,i_min);
  }

  void RobotXController::cmdVelCallback(const geometry_msgs::Twist& msg)
  {
    if (isRunning())
    {
  ...
  void RobotXController::twistCallback(const geometry_msgs::Twist& msg)
  {
    if (isRunning())
    {
...
PLUGINLIB_EXPORT_CLASS(robotx_controller::RobotXController, controller_interface::ControllerBase);
