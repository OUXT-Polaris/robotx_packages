...
void UsvWindPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  ROS_INFO_STREAM("Wind velocity vector = "
                  << param_wind_velocity_vector_.x << " , "
                  << param_wind_velocity_vector_.y << " , "
                  << param_wind_velocity_vector_.z);

  if (_sdf->HasElement("wind_coeff_vector")) {
    param_wind_coeff_vector_ =
        _sdf->GetElement("wind_coeff_vector")->Get<math::Vector3>();
  }
  else {
    param_wind_coeff_vector_ = math::Vector3(0, 0, 0);
  }
  ROS_INFO_STREAM("Wind coefficient vector = "
                  << param_wind_coeff_vector_.x << " , "
  ...
  this->spinner_thread_ =
      new boost::thread(boost::bind(&UsvWindPlugin::spin, this));
...
void UsvWindPlugin::UpdateChild()
{
  ...
  math::Vector3 relative_wind =
      link_->GetWorldPose().rot.GetInverse().RotateVector(
          param_wind_velocity_vector_);
