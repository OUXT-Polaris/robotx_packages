...
void UsvWindPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  ROS_INFO_STREAM("Wind velocity vector = " << param_wind_velocity_vector_.x << " , "
                                            << param_wind_velocity_vector_.y << " , "
                                            << param_wind_velocity_vector_.z);
  ...
  ROS_INFO_STREAM("Wind coefficient vector = " << param_wind_coeff_vector_.x << " , "
                                               << param_wind_coeff_vector_.y << " , "
                                               << param_wind_coeff_vector_.z);
  ...
  this->updateConnection =
      event::Events::ConnectWorldUpdateBegin(boost::bind(&UsvWindPlugin::UpdateChild, this));
}
void UsvWindPlugin::UpdateChild() {
  // Wind
  // Transform wind from world coordinates to body coordinates
  math::Vector3 relative_wind =
      link_->GetWorldPose().rot.GetInverse().RotateVector(param_wind_velocity_vector_);
  ...
  math::Vector3 wind_force(param_wind_coeff_vector_.x * relative_wind.x * abs(relative_wind.x),
