...
class twist_sensor_plugin : public ModelPlugin {
 ...
 public:
  ...
  {
    this->model = _parent;
    this->LoadParams(sdf, "targetLink", this->target_link,
                     std::string("base_link"));
    this->LoadParams(sdf, "sensorNoiseVariance", this->sensor_noise_variance,
                     0.1);
    ...
    this->update_connection = event::Events::ConnectWorldUpdateBegin(
        boost::bind(&twist_sensor_plugin::OnUpdate, this, _1));
    boost::thread publisher_thread(
        boost::bind(&twist_sensor_plugin::publish_twist, this));
  }
 ...
 public:
  void publish_twist()
  {
    ...
    std::normal_distribution<double> distribution(0,
                                                  this->sensor_noise_variance);
    while (ros::ok()) {
   ...
   */
  template <typename T>
  bool LoadParams(sdf::ElementPtr sdf,
                  std::string key,
                  T &param,
                  T default_param)
  {
    ...
    if (!sdf->HasElement(key)) {
      param = default_param;
      ROS_INFO_STREAM("unable to get "
                      << key << " ,set default_value = " << default_param);
      return false;
    }
    else {
      param_str = sdf->GetElement(key)->GetValue()->GetAsString();
