    ...
    if (this->sdf->HasElement("link")) {
        ...
        for (sdf::ElementPtr linkElem = this->sdf->GetElement("link"); linkElem;
            ...
            if (this->volPropsMap.count(id) != 0) {
                gzwarn << "Properties for link [" << name
            ...
            if (linkElem->HasElement("center_of_volume")) {
                ...
                gzwarn
                    << "Required element center_of_volume missing from link ["
            ...
            */
            if (linkElem->HasElement("area")) {
                ...
                gzwarn << "Required element 'area' missing from element link ["
                       << name << "] in BuoyancyPlugin SDF" << std::endl;
                continue;
            }
            ...
            if (linkElem->HasElement("height")) {
                ...
                gzwarn
                    << "Required element 'height' missing from element link ["
                    << name << "] in BuoyancyPlugin SDF" << std::endl;
                continue;
            }
        }
...
void BuoyancyPlugin::OnUpdate() {
    ...
    for (auto ii = 0u; ii < buoyLinks.size(); ii++) {
        ...
        GZ_ASSERT(volume >= 0,
                  "Nonpositive volume found in volume properties!");
